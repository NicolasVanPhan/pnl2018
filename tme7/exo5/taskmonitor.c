...
unsigned frequency = 20; /* sampling frequency */
...
static mempool_t	*ts_mempool;

/* -------------------------------------------------------------------------- */
/* ---------- TME7 Exercice 5 : Mempool ------------------------------------- */
/* -------------------------------------------------------------------------- */

void ts_release(struct kref *kref)
{
	struct task_sample	*pts;
	struct list_head	*plh;

	pts = container_of(kref, struct task_sample, refcount);
	plh = &pts->list;
...
unsigned long taskmon_scan(struct shrinker* s, struct shrink_control *sc)
{
	...
	while (!list_empty(&tm->phantom.list)) {
		...
		if (kref_put(&pts->refcount, ts_release) == 0)
			break;
		tm->nsamples--;
