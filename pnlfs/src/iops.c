...
int
pnlfs_iset(struct super_block *sb, struct inode *inode, int sync);

struct inode
*pnlfs_iget(struct super_block *sb, unsigned long ino);
...
static int
pnlfs_mkdir(struct inode *dir, struct dentry *de, umode_t mode);

static int
pnlfs_rmdir(struct inode *dir, struct dentry *de);
